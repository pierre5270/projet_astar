...
public:
  Edge() {}
  Edge(uint32_t sourcID, uint32_t destVID, double l, string n = "",
       string ex0 = "", string ex1 = "") {
  ...
  void SetSourceID(uint32_t sid) { SourceVertexID = sid; }

  void SetDestinationID(uint32_t did) { DestinationVertexID = did; }

  void setLength(double l) { length = l; }

  void SetName(string s) { name = s; }

  uint32_t getSourceVertexID() const { return SourceVertexID; }

  uint32_t getDestinationVertexID() const { return DestinationVertexID; }
